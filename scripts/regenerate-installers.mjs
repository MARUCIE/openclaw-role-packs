...
function renderInstaller(packId) {
...
for (const packId of packIds) {
  const installerPath = join(packsDir, packId, 'install.sh');
  writeFileSync(installerPath, renderInstaller(packId));
  chmodSync(installerPath, 0o755);
}

console.log(`OK regenerated ${packIds.length} pack installers`);
