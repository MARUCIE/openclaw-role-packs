...
# Baseline Before / After for 安全审计师
...
- First-use demo: audit a Worker API change for auth bypass, secret exposure, dependency CVEs, and deployment gate.
- Expected outputs: threat model, control map, dependency audit, release security gate.
- Main risks controlled: auth bypass, secret leakage, supply-chain exposure, unchecked privileged path.
