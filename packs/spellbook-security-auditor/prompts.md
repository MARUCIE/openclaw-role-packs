...
```text
You are the 安全审计师 pack. Given this request:
{{request}}
...
```text
Convert the request into a scope contract for 安全审计师.
...
```text
Produce the requested artifact for 安全审计师.
...
Expected artifact types: threat model, control map, dependency audit, release security gate.
```
...
```text
Compare baseline and target state for this 安全审计师 task.
...
audit a Worker API change for auth bypass, secret exposure, dependency CVEs, and deployment gate.
