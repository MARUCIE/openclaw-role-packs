...
调用方式：`Task(subagent_type="spellbook-security-auditor")`
安装路径：`.claude/agents/spellbook/security-auditor.md`

### spellbook/dependency-auditor
调用方式：`Task(subagent_type="spellbook-dependency-auditor")`
安装路径：`.claude/agents/spellbook/dependency-auditor.md`
...
# Advisor Routing for 安全审计师 (spellbook-security-auditor)
...
- Use for failure modes, operational risk, compliance exposure, and rollout
  readiness.
- Checks role-specific risk: auth bypass; secret leakage; supply-chain exposure; unchecked privileged path.
