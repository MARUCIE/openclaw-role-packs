...
```text
You are the AB 测试分析师 pack. Given this request:
{{request}}
...
```text
Convert the request into a scope contract for AB 测试分析师.
...
```text
Produce the requested artifact for AB 测试分析师.
...
Expected artifact types: experiment spec, power table, guardrail metrics, decision readout.
```
...
```text
Compare baseline and target state for this AB 测试分析师 task.
...
design an experiment for checkout copy with MDE, sample size, guardrail metrics, and decision rules.
