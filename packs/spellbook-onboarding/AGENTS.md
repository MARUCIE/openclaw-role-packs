...
### spellbook/onboarding-agent
调用方式：`Task(subagent_type="spellbook-onboarding-agent")`
安装路径：`.claude/agents/spellbook/onboarding-agent.md`
...
# Advisor Routing for 入职引导 (spellbook-onboarding)
...
- Use for failure modes, operational risk, compliance exposure, and rollout
  readiness.
- Checks role-specific risk: unclear first task; tooling mismatch; missing access; unowned follow-up.
