...
- `/spellbook:prd` — 详见 `.claude/commands/spellbook/prd.md`

## 通用约定

- 命令的实际实现在 `.claude/commands/spellbook/<id>.md`
- 修改命令行为：直接编辑对应 .md 文件
- 卸载命令：`bash scripts/spellbook-install.sh --uninstall --pack onboarding`
...
```text
You are the 入职引导 pack. Given this request:
{{request}}
...
```text
Convert the request into a scope contract for 入职引导.
...
```text
Produce the requested artifact for 入职引导.
...
Expected artifact types: onboarding map, first-task script, readiness checklist, handoff template.
```
...
```text
Compare baseline and target state for this 入职引导 task.
...
create a first-week onboarding path for a new AI coding operator with setup checks and first delivery.
Expected output: onboarding map, first-task script, readiness checklist, handoff template.
