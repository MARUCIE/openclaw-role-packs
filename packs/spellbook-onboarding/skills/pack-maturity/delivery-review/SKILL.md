---
name: spellbook-onboarding-delivery-review
...
- Use when operating the 入职引导 pack.
...
Delivery Review Workflow 用来把 入职引导 场景里的任务输入转成可执行的流程、检查清单和交付物。

Delivery Review Workflow

它的价值在于让 产品职能线 在 Claude Code、Codex、Gemini、Hermes 或 OpenClaw 中复用同一套岗位能力，而不是依赖一次性的聊天提示词。
