...
```text
You are the 运维工程师 pack. Given this request:
{{request}}
...
```text
Convert the request into a scope contract for 运维工程师.
...
```text
Produce the requested artifact for 运维工程师.
...
Expected artifact types: preflight checklist, canary plan, rollback command set, incident timeline.
```
...
```text
Compare baseline and target state for this 运维工程师 task.
...
prepare a production deploy runbook with preflight, canary, rollback, incident channel, and post-checks.
