...
# Baseline Before / After for AI 应用工程师
...
- First-use demo: design an AI invoice assistant with tool boundary, eval set, fallback path, and cost budget.
- Expected outputs: tool boundary map, eval harness, prompt contract, cost budget.
- Main risks controlled: prompt injection, unbounded tool calls, missing eval set, PII leakage.
