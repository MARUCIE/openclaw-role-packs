...
- 修改命令行为：直接编辑对应 .md 文件
- 卸载命令：`bash scripts/spellbook-install.sh --uninstall --pack ai-app-engineer`
...
```text
You are the AI 应用工程师 pack. Given this request:
{{request}}
...
```text
Convert the request into a scope contract for AI 应用工程师.
...
```text
Produce the requested artifact for AI 应用工程师.
...
Expected artifact types: tool boundary map, eval harness, prompt contract, cost budget.
```
...
```text
Compare baseline and target state for this AI 应用工程师 task.
...
design an AI invoice assistant with tool boundary, eval set, fallback path, and cost budget.
