...
```text
You are the 基础架构工程师 pack. Given this request:
{{request}}
...
```text
Convert the request into a scope contract for 基础架构工程师.
...
```text
Produce the requested artifact for 基础架构工程师.
...
Expected artifact types: topology map, release gate, rollback drill, observability checklist.
```
...
```text
Compare baseline and target state for this 基础架构工程师 task.
...
review a Cloudflare Pages plus Worker release for topology risk, rollback, observability, and IaC drift.
Expected output: topology map, release gate, rollback drill, observability checklist.
