...
```text
You are the 算法工程师 pack. Given this request:
{{request}}
...
```text
Convert the request into a scope contract for 算法工程师.
...
```text
Produce the requested artifact for 算法工程师.
...
Expected artifact types: experiment contract, metric ladder, ablation table, inference rollout gate.
```
...
```text
Compare baseline and target state for this 算法工程师 task.
...
triage invoice OCR model regression with offline metric, ablation table, drift hypothesis, and rollout gate.
