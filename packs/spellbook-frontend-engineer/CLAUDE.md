# 前端工程师 · 配置 (Claude Code)

> Spellbook role pack adapted from kid-sid/claude-spellbook into AI-Fleet,
> bridged into Agent Foundry as job pack `spellbook-frontend-engineer`.

## 角色定位

WCAG/ARIA 无障碍 + Angular + Tailwind + 3 种语言 lint 配置。

英文版：WCAG/ARIA accessibility + Angular + Tailwind + 3-language lint configs.

## 适用场景

当 Claude Code 会话需要扮演 **前端工程师** 角色时，激活这套配置。Claude 将
按以下技能集合自动触发对应专项行为：

- **Skills**: accessibility, angular, tailwind
...
```bash
cd /path/to/AI-Fleet
bash scripts/spellbook-install.sh --pack frontend-engineer
```
