...
```text
You are the 投资分析师 pack. Given this request:
{{request}}
...
```text
Convert the request into a scope contract for 投资分析师.
...
```text
Produce the requested artifact for 投资分析师.
...
Expected artifact types: investment memo, scenario model, risk matrix, diligence checklist.
```
...
```text
Compare baseline and target state for this 投资分析师 task.
...
build an investment memo for an AI tooling company with thesis, scenarios, risks, and diligence plan.
