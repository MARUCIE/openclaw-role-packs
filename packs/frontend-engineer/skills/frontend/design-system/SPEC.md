...
帮你从零写出一份合格的单系统设计文档，让你不再面对空白页发呆——它先读完所有上下文，再带着你一节一节往下问，最后落成一份工程师能直接照着实现的 GDD（Game Design Document，游戏设计文档）。

## 怎么用

1. 把系统名扔给它，比如"战斗系统""库存""对话"，它会自动转成规范的文件名。
2. 它先把项目主线、系统索引、上下游依赖文档都读一遍，把全局背景吃透再开口提问。
3. 它按 8 节顺序带你走：概览、玩家体验、规则、公式、边界情况、依赖、可调参数、验收标准，每节问完就先落盘再继续。
4. 它边写边给你交叉对照：你定的公式会不会和上游冲突、你的输出能不能满足下游的预期，问题当场提醒。
5. 全部写完它会自动跑一遍 design-review 自检，把缺漏和不一致的点列出来给你最后过一遍。
