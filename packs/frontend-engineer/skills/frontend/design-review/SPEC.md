...
帮你在设计稿交给工程师之前，把这份文档当成"准生证"过一遍，让你提前发现规则含糊、公式缺漏、跨系统冲突这些常见问题，避免开发到一半才返工。

## 怎么用

1. 把要审的设计文档路径丢给它，比如战斗系统、库存系统这种单一系统的 GDD（Game Design Document，游戏设计文档）。
2. 它会读完目标文档，再扫一遍主项目说明和相关联的兄弟系统文档，把上下文补齐。
3. 它按 8 个必备小节给你打分：概览、玩家体验、规则细节、公式、边界情况、依赖、可调参数、验收标准，缺哪个直接列出来。
4. 它会顺手检查公式和描述是否自洽、边界情况会不会和主规则打架、依赖关系是否双向贯通。
5. 它最后输出一份分级评审：完整度多少、内部一致性如何、可实现度怎样、跨系统是否冲突，每条问题都给改进建议。
